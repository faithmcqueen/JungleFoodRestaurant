...
// ----- TOGGLE CONTENT (GALLERY & FORM) -----//
	...
	if (formHandle){
		...
		function processForm(){
			...
			var eventNameValue = formHandle.inEventName.value;
			...
			if(nameValue === "" || nameValue === null) {
				$("#events_username").after("<div class=\"field-error pure-u-1-4\">Please enter your name.</div>")
				errorFlag = true;
			}
			...
			if (phoneValue === "" || phoneValue === null || !phoneRegex.test(phoneValue)) {
				$("#events_phonenumber").after("<div class=\"field-error pure-u-1-4\">Please enter a valid phone number.</div>")
				errorFlag = true;
			}
			...
			if (emailValue == "" || emailValue === null || !emailRegex.test(emailValue)) {
				$("#events_email").after("<div class=\"field-error pure-u-1-4\">Please enter a valid email address.</div>");
				errorFlag = true;
			}

			if (guestNumberValue === "" || guestNumberValue === null || guestNumberValue < 0 || guestNumberValue > 150){
				$("#events_guests").after("<div class=\"field-error pure-u-1-4\">Please enter the approx. number of guests. Max capacity: 150</div>");
				errorFlag = true;
			}

			if (eventLocationValue === "none") {
				$("#events_locations").after("<div class=\"field-error pure-u-1-4\">Please select a location.</div>")
			}
